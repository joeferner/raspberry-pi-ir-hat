...
const void GPIO::setMode(gpio::Mode mode) const {
  LL_GPIO_SetPinMode(this->port, this->pin, (uint32_t)mode);
}

const void GPIO::setPull(gpio::Pull pull) const {
  LL_GPIO_SetPinPull(this->port, this->pin, (uint32_t)pull);
}
...
const void GPIO::resetOutputPin() const {
  LL_GPIO_ResetOutputPin(this->port, this->pin);
}

const void GPIO::setSpeed(gpio::Speed speed) const {
  LL_GPIO_SetPinSpeed(this->port, this->pin, (uint32_t)speed);
}
